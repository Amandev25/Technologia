...
# --- NLTK Dictionary Setup ---
try:
    nltk.data.find('corpora/cmudict.zip')
except nltk.downloader.DownloadError:
    print("Downloading NLTK's CMU Pronouncing Dictionary (one-time setup)...")
...
class PronunciationAssistant:
    def __init__(self):
        # English-specific tools
        self.syllable_dic_en = pyphen.Pyphen(lang='en_US')
        self.g2p_en = G2p()
        self.cmu_dict_en = cmudict.entries()

    def _detect_language(self, word):
        """Detects the language of the word based on its script."""
        # Devanagari (Hindi) script unicode range
    ...
    def _get_hindi_aksharas(self, word):
        """Splits a Hindi word into its phonetic components (aksharas)."""
        return re.findall(r'[\u0900-\u097F][\u093B-\u094D\u0962\u0963]*', word) or [word]
        
    def _get_telugu_aksharas(self, word):
        """Splits a Telugu word into its phonetic components (aksharas)."""
        return re.findall(r'[\u0c00-\u0c7f][\u0c3e-\u0c56]*', word) or [word]

    def _get_english_syllables(self, word):
        """Breaks an English word into syllables."""
        return self.syllable_dic_en.inserted(word).split('-')

    def _find_simple_english_word(self, syllable):
        """Finds a simple, common word with a similar sound for an English syllable."""
        pre_selected = {'tion': 'nation', 'pro': 'promote', 'pho': 'photo'}
        if syllable.lower() in pre_selected:
            return pre_selected[syllable.lower()]
        try:
            ...
            for word, phonemes in self.cmu_dict_en:
                if len(self._get_english_syllables(word)) == 1 and word.isalpha():
                    ...
                    if phonemes_base == syllable_phonemes_base:
                        return word
        except Exception:
            return None
        return syllable
    ...
    def generate_audio(self, text, filepath, lang='en'):
        """Generates and saves an MP3 audio file using Google TTS."""
        try:
    ...
    def breakdown_word(self, word):
        """The main method, now handling English, Hindi, and Telugu."""
        ...
        result = {
            "word": word, "language": lang_name, "full_audio_path": full_audio_path, "syllables": []
        }

        if lang_name == 'english':
            components = self._get_english_syllables(word)
        elif lang_name == 'hindi':
            components = self._get_hindi_aksharas(word)
        elif lang_name == 'telugu':
            components = self._get_telugu_aksharas(word)
        else:
            components = [word]
        ...
        for i, comp in enumerate(components):
            ...
            result["syllables"].append({
                "text": comp, "audio_path": comp_audio_path, "example_word_sound": example_word
            })
